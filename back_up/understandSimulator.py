...
def sample_data(x0, bounds, name_of_bounds, num_of_samples=10, filename = "results/torque_data.csv"):
...
def find_model_for_each_para_from_csv(data, name_of_bounds):
    ...
    for param_name in name_of_bounds:
        ...
        plt.grid(True)
        plt.savefig(f'results/{param_name}_torque_down_model.png')
        ...
        plt.subplots_adjust(bottom=0.2)
        plt.grid(True)
        plt.savefig(f'results/{param_name}_torque_up_model.png')
...
def plot_parameter_influence(models_down, models_up, bounds, name_of_bounds):
    ...
    ax2.legend()
    plt.tight_layout()
    plt.savefig('results/parameter_influence.png')
    plt.close()
...
filename = "results/torque_data.csv"
